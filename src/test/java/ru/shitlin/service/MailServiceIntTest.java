...
public class MailServiceIntTest {
    ...
    @Test
    public void testSendGreetingsEmail() throws Exception {
        Client client = new Client();
        client.setFirstName("Йенн");
        client.setLastName("Фредерикс");
        client.setEmail("john.doe@example.com");
        mailService.sendGreetingsEmail(client, "greetings");
        verify(javaMailSender).send(messageCaptor.capture());
        MimeMessage message = messageCaptor.getValue();
        assertThat(message.getAllRecipients()[0].toString()).isEqualTo(client.getEmail());
        assertThat(message.getFrom()[0].toString()).isEqualTo("test@localhost");
        assertThat(message.getContent().toString()).isNotEmpty();
        assertThat(message.getDataHandler().getContentType()).isEqualTo("text/html;charset=UTF-8");
    }
    ...
    @Test
    public void testSendStartLearningEmail() throws Exception {
        ...
        contract.setEndDate(LocalDate.now().plusDays(10L));
        contract.setMentor(mentor);
        contract.setAgreement(agreement);

        mailService.sendStartLearningEmail(contract, "javaee");
        verify(javaMailSender).send(messageCaptor.capture());
        MimeMessage message = messageCaptor.getValue();
        assertThat(message.getAllRecipients()[0].toString()).isEqualTo(client.getEmail());
        assertThat(message.getFrom()[0].toString()).isEqualTo("test@localhost");
