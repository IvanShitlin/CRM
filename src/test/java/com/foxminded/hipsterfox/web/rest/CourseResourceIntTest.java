...
import com.foxminded.hipsterfox.service.CourseQueryService;
import com.foxminded.hipsterfox.service.CourseService;
import com.foxminded.hipsterfox.service.dto.CourseDTO;
import com.foxminded.hipsterfox.service.mapper.CourseMapper;
...
public class CourseResourceIntTest {
    ...
    public void getAllCoursesByNameIsEqualToSomething() throws Exception {
        String filter = "name.equals=";
        int databaseSizeBeforeTest = getCoursesCount(filter + DEFAULT_NAME);

        // Initialize the database
        courseRepository.saveAndFlush(course);

        // Get all the courseList where name equals to DEFAULT_NAME
        defaultCourseShouldBeFound(filter + DEFAULT_NAME,  databaseSizeBeforeTest + 1);

        // Get all the courseList where name equals to UPDATED_NAME
        defaultCourseShouldNotBeFound(filter + UPDATED_NAME);
    }
    ...
    public void getAllCoursesByNameIsInShouldWork() throws Exception {
        ...
        int databaseSizeBeforeTest = getCoursesCount(filter  + DEFAULT_NAME + "," + UPDATED_NAME);

        // Initialize the database
        courseRepository.saveAndFlush(course);

        // Get all the courseList where name in DEFAULT_NAME or UPDATED_NAME
        defaultCourseShouldBeFound(filter  + DEFAULT_NAME + "," + UPDATED_NAME, databaseSizeBeforeTest + 1);
    ...
    public void getAllCoursesByNameIsNullOrNotNull() throws Exception {
        String filter = "name.specified";
        int databaseSizeBeforeTest = getCoursesCount(filter + "=true");

        // Initialize the database
        courseRepository.saveAndFlush(course);

        // Get all the courseList where name is not null
        defaultCourseShouldBeFound(filter + "=true", databaseSizeBeforeTest + 1);

        // Get all the courseList where name is null
        defaultCourseShouldNotBeFound(filter + "=false");
    }

    @Test
    @Transactional
    public void getAllCoursesByDescriptionIsEqualToSomething() throws Exception {
        String filter = "description.equals=";
        int databaseSizeBeforeTest = getCoursesCount(filter + DEFAULT_DESCRIPTION);

        // Initialize the database
        courseRepository.saveAndFlush(course);

        // Get all the courseList where description equals to DEFAULT_DESCRIPTION
        defaultCourseShouldBeFound(filter + DEFAULT_DESCRIPTION, databaseSizeBeforeTest + 1);

        // Get all the courseList where description equals to UPDATED_DESCRIPTION
        defaultCourseShouldNotBeFound(filter + UPDATED_DESCRIPTION);
    }

    @Test
    @Transactional
    public void getAllCoursesByDescriptionIsInShouldWork() throws Exception {
        String filter = "description.in=";
        int databaseSizeBeforeTest = getCoursesCount(filter + DEFAULT_DESCRIPTION + "," + UPDATED_DESCRIPTION);

        // Initialize the database
        courseRepository.saveAndFlush(course);

        // Get all the courseList where description in DEFAULT_DESCRIPTION or UPDATED_DESCRIPTION
        defaultCourseShouldBeFound(filter  + DEFAULT_DESCRIPTION + "," + UPDATED_DESCRIPTION, databaseSizeBeforeTest + 1);
    ...
    public void getAllCoursesByDescriptionIsNullOrNotNull() throws Exception {
        String filter = "description.specified";
        int databaseSizeBeforeTest = getCoursesCount(filter + "=true");
    ...
    public void getAllCoursesByEmailTemplateIsEqualToSomething() throws Exception {
        String filter = "emailTemplate.equals=";
        int databaseSizeBeforeTest = getCoursesCount(filter + DEFAULT_EMAIL_TEMPLATE);

        // Initialize the database
        courseRepository.saveAndFlush(course);

        // Get all the courseList where emailTemplate equals to DEFAULT_EMAIL_TEMPLATE
        defaultCourseShouldBeFound(filter + DEFAULT_EMAIL_TEMPLATE, databaseSizeBeforeTest + 1);

        // Get all the courseList where emailTemplate equals to UPDATED_EMAIL_TEMPLATE
        defaultCourseShouldNotBeFound(filter + UPDATED_EMAIL_TEMPLATE);
    }

    @Test
    @Transactional
    public void getAllCoursesByEmailTemplateIsInShouldWork() throws Exception {
        String filter = "emailTemplate.in=";
        int databaseSizeBeforeTest = getCoursesCount(filter  + DEFAULT_EMAIL_TEMPLATE + "," + UPDATED_EMAIL_TEMPLATE);

        // Initialize the database
        courseRepository.saveAndFlush(course);

        // Get all the courseList where emailTemplate in DEFAULT_EMAIL_TEMPLATE or UPDATED_EMAIL_TEMPLATE
        defaultCourseShouldBeFound(filter  + DEFAULT_EMAIL_TEMPLATE + "," + UPDATED_EMAIL_TEMPLATE, databaseSizeBeforeTest + 1);
    ...
    public void getAllCoursesByEmailTemplateIsNullOrNotNull() throws Exception {
        String filter = "emailTemplate.specified";
        int databaseSizeBeforeTest = getCoursesCount(filter + "=true");
    ...
    public void getAllCoursesByMentorsIsEqualToSomething() throws Exception {
        ...
        String filter = "mentorsId.equals=";
        int databaseSizeBeforeTest = getCoursesCount(filter + mentorsId);
        ...
        defaultCourseShouldBeFound(filter  + mentorsId, databaseSizeBeforeTest + 1);

        // Get all the courseList where mentors equals to mentorsId + 1
        defaultCourseShouldNotBeFound(filter + (mentorsId + 1));
    }

    /**
     * Executes the search, and checks that the default entity is returned
     */
    private void defaultCourseShouldBeFound(String filter, int count) throws Exception {
    ...
    private int getCoursesCount(String filter) throws Exception {
        return Integer
            .decode(restCourseMockMvc.perform(get("/api/courses/count?sort=id,desc&" + filter))
            .andReturn()
            .getResponse()
            .getContentAsString());
    }
